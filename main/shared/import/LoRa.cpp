...
void LoRa::initializeSPI( int mosi, int miso, int clk, int cs )
{
    ...
    ESP_ERROR_CHECK(ret);
    ESP_LOGI(LORATAG,"Bus Init: %d", ret);
    ...
    ESP_ERROR_CHECK(ret);
    ESP_LOGI(LORATAG,"Add device: %d", ret);
...
void LoRa::initialize( int power )
{
    ...
    ESP_LOGI(LORATAG, "Version: [%d]", version );
    setFrequency( 915E6 );
...
void LoRa::setTxPower(int8_t level, int8_t outputPin)
{
	...
	else
	{
		...
		ESP_LOGI(LORATAG, "RegPAConfig: [%02x]", paConfig);
		ESP_LOGI(LORATAG, "RegPADAC: [%02x]", paDAC);
...
size_t LoRa::write(const uint8_t *buffer, size_t size)
{
  	...
  	//ESP_LOGI(LORATAG, "Current: %d Size: %d" , currentLength, size );
  	...
  	//ESP_LOGI(LORATAG, "New Size: %d" , size );
...
size_t LoRa::write(const std::string& buffer)
{
	return write((uint8_t*)buffer.data(), buffer.size());
}

void LoRa::dumpRegisters()
{
  	for (int i = 0; i < 128; i++)
		ESP_LOGI(LORATAG, "%02x: %02x", i, readRegister(i) );
}

int LoRa::available()
{
	//ESP_LOGI(LORATAG, "RX Nbr Bytes: [%d]", readRegister(REG_RX_NB_BYTES) );
...
int LoRa::handleDataReceived( std::string& msg )
{
	...
	if ((irqFlags & IRQ_PAYLOAD_CRC_ERROR_MASK) == 0)
	{
		...
		ESP_LOGI(LORATAG, "In handleDataReceived: len: %d fifo_addr: %d rx_addr: %d",
				packetLength,
...
{
    //ESP_LOGI(LORATAG,"Writing Register [%02x]=[%02x]", reg, data);
	reg = reg | 0x80;
	...
	if (err != ESP_OK)
	    ESP_LOGE(LORATAG, "Error adding SPI device: %s", esp_err_to_name(err));
}
...
//    ESP_LOGI(LORATAG,"Reading Register [%02x]=[%02x]", reg, result);
