...
DHT::DHT(gpio_num_t pin, uint8_t type, uint8_t count) {
  ...
  _pin = pin;
  _type = type;
  //_maxcycles = 1 / portTICK_PERIOD_MS;
...
void DHT::begin(uint8_t usec) {
  ...
  c.pin_bit_mask = BIT(static_cast<size_t>(_pin));
  c.intr_type = GPIO_INTR_DISABLE;
  if (gpio_config(&c) != ESP_OK) throw -1;
...
float DHT::readTemperature(bool S, bool force) {
  ...
  if (read(force)) {
    ...
    case DHT22:
    case DHT21:
      f = ((uint32_t)(data[2] & 0x7F)) << 8 | data[3];
      f *= 0.1;
      if (data[2] & 0x80) {
        f *= -1;
      }
...
uint32_t DHT::expectPulse(bool level) {
  ...
  while (gpio_get_level(_pin) == level) {
    if ((stor = static_cast<uint32_t>(esp_timer_get_time() - nowus)) > 1000) return TIMEOUT; // Exceeded timeout, fail.
  }
